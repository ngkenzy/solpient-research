...
export default async function ResearchRequestPage({
  ...
  const [companiesR,requestsR]=await Promise.all([
    supabase.from("companies").select("id,ticker,company_name").order("ticker"),
    supabase.from("research_demand_requests")
      .select("id,company_id,request_type,priority,question,reason,updated_at")
    ...
    <div className={styles.page}>
      ...
      <main className={styles.main}>
        <section className={styles.hero}>
          ...
          <p>
            Your request is private. Solpient aggregates demand for internal research triage, but requests never change rankings, readiness, or Solpient 100 eligibility automatically.
          </p>
        </section>
