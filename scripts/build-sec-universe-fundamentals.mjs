...
tickerMap.sort((a,b)=>String(a.ticker).localeCompare(String(b.ticker)));
const selected=maxSymbols?tickerMap.slice(0,maxSymbols):tickerMap;
...
for(let i=0;i<selected.length;i++){
  ...
  if(!factsPath){skipped.missing_companyfacts++;continue;}
  try{
    ...
    if(!row){skipped.no_normalized_facts++;continue;}
    securities.push(row);
  }catch(error){
    skipped.parse_error++;
    console.error("SEC universe normalize failed",meta.ticker,error instanceof Error?error.message:String(error));
  }
