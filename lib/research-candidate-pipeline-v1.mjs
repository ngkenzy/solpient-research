...
} from "./universe-screening-engine-v1.mjs";
import {
...
export const RESEARCH_CANDIDATE_PIPELINE_VERSION="research-candidate-pipeline-v1";
...
export function valuationProfileForScreen(screenResult={}){
  ...
  return SCREEN_TO_VALUATION_PROFILE[profile]??"generic";
}
...
export function valuationPreflight(screenResult={},input={}){
  const industryModule=input.industryModule??valuationProfileForScreen(screenResult);
...
function nextActions({eligible,companyExists,preflight,valuation,decision}){
  ...
  if(!preflight.complete){
    actions.push({
      ...
      reason:"Valuation V3 does not impute missing assumptions or relative-valuation anchors.",
...
export function buildResearchCandidatePipeline({
  ...
  const valuation=eligible?valueCompanyV3(valuationInputWithProfile):null;
