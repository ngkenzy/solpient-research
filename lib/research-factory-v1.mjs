...
export const RESEARCH_FACTORY_AUTOMATION_VERSION="research-factory-v1.1";
export const RESEARCH_FACTORY_SAFE_AUTOMATION_STAGES=[
  "evidence_ingestion",
  "baseline_draft",
  "research_draft",
];
export const RESEARCH_FACTORY_HUMAN_REVIEW_ACTION_TYPES=[
  "identity_review",
  "industry_module_review",
  "repair_review",
];
...
const actionTypes=(item)=>new Set(
  (Array.isArray(item?.next_actions)?item.next_actions:[])
    .map(action=>String(action?.type??""))
    .filter(Boolean)
);

export function isResearchFactoryReviewGate(item={}){
  if(item?.status==="complete"||item?.stage==="complete")return true;
  if(item?.stage==="pipeline_refresh")return true;
  if(item?.status!=="needs_review")return false;
  if(["valuation_review","research_review"].includes(item?.stage))return true;
  const types=actionTypes(item);
  return RESEARCH_FACTORY_HUMAN_REVIEW_ACTION_TYPES.some(type=>types.has(type));
}

export function isSafeResearchFactoryAutomationItem(item={}){
  if(!RESEARCH_FACTORY_SAFE_AUTOMATION_STAGES.includes(item?.stage))return false;
  if(!["queued","running"].includes(item?.status))return false;
  if(Number(item?.manual_review_count??0)>0)return false;
  const types=actionTypes(item);
  return !RESEARCH_FACTORY_HUMAN_REVIEW_ACTION_TYPES.some(type=>types.has(type));
}

export function summarizeResearchFactoryQueue(items=[]){
  const summary={
    ...
    completed_count:0,
    unresolved_count:0,
    all_at_review_gate:false,
    stage_status_counts:{},
  };
  for(const item of items){
    const key=String(item?.stage??"unknown")+"|"+String(item?.status??"unknown");
    summary.stage_status_counts[key]=(summary.stage_status_counts[key]??0)+1;
    if(isSafeResearchFactoryAutomationItem(item))summary.safe_queue_count++;
    if(isResearchFactoryReviewGate(item))summary.review_gate_count++;
    if(item?.status==="blocked")summary.blocked_count++;
    if(item?.status==="running")summary.running_count++;
    if(item?.status==="complete"||item?.stage==="complete")summary.completed_count++;
  }
  summary.unresolved_count=Math.max(
    0,
    summary.candidate_count-summary.review_gate_count-summary.blocked_count
  );
  summary.all_at_review_gate=
    summary.candidate_count>0&&
    summary.review_gate_count===summary.candidate_count&&
    summary.blocked_count===0&&
    summary.safe_queue_count===0&&
