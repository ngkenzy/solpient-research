...
export function directionObservation(row={}){
  ...
  const probability=probabilityForecastObservation(row);
  if(probability)return probability.class_correct;

  const metric=String(row.metric_key??"").toLowerCase();
  const directionalMetric=["price_return_pct","relative_return_pct","benchmark_excess_return"].includes(metric);
  if(directionalMetric){
    const predicted=n(row.predicted_value),actual=n(row.actual_value);
    if(predicted==null||actual==null)return null;
    return Math.sign(predicted)===Math.sign(actual);
  }

  const text=String(row.predicted_text??"").toLowerCase();
  const actual=n(row.actual_value);
  if(actual!=null&&text.includes("outperform"))return actual>0;
  if(actual!=null&&text.includes("underperform"))return actual<0;
  return null;
}
