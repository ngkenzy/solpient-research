...
export function validateResearchStandardV2(payload){
 ...
 for(const module of modules){
   ...
   if(def.requirePeerValuation){
     const peerMethod=(Array.isArray(payload?.valuation_analysis?.methods)?payload.valuation_analysis.methods:[]).find((row)=>row?.method==="peer_valuation");
     const passed=peerMethod?.status==="applied";
     decisionGradeChecks.push(passed);
     if(!passed)decisionGradeBlockers.push("Decision-grade "+module+" research requires an applied normalized peer valuation.");
   }
 }
