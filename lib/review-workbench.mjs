import { validateResearchStandard } from "./research-standard-v1.mjs";
...
export function applyReviewPatch(basePayload, reviewPatch = {}) {
  ...
  for (const key of ["research", "business_assessment", "scores", "valuations", "ranking", "financial_metrics"]) {
  ...
  for (const key of ["risk_register", "expected_return_scenarios", "thesis_variables"]) {
...
export function validatePromotionReadiness(payload) {
  ...
  if (!standard.valid) blockers.push("Research Standard v1 structural validation has not passed.");
  if (standard.status !== "complete") blockers.push("Research Standard v1 status must be complete before promotion.");
