...
const BANK_MODULES = new Set(["financial_bank", "financial_insurance"]);
const REIT_MODULES = new Set(["reit", "real_estate"]);
...
function companyOverview(pack, metrics, docs) {
  const name = pack.company?.company_name ?? pack.company?.ticker;
  const sector = pack.company?.sector;
  const refs = [];
  const parts = [`${name} is in the evidence pack`];
  if (docs[0]) {
  ...
  if (!refs.length) {
    return insufficient(
      "No metric or source_document evidence_item is available to ground a company overview.",
    );
  }
  const module = moduleOf(pack);
  if (!isReviewed(pack)) {
...
function risks(pack, metrics, docs) {
  ...
  const gapNote = (pack.evidence_gaps ?? [])
    .filter((gap) => gap.kind === "missing_sector_metric")
    .map((gap) => gap.metric_key)
    .slice(0, 6);
...
function unknowns(pack) {
  ...
  const gaps = pack.evidence_gaps ?? [];
  if (!cov.length && !gaps.length) {
    return insufficient("The pack lists no coverage rows or evidence_gaps.");
  }
  if (!cov.length) {
    return insufficient(
      `Evidence gaps are listed (${gaps.map((g) => g.kind + (g.metric_key ? ":" + g.metric_key : "")).join(", ")}) but gaps are not evidence_items and cannot be cited. No coverage evidence_item exists to ground this section.`,
    );
  }
  const gapText = gaps.length
    ? gaps
        .slice(0, 8)
        .map((g) => g.metric_key ?? g.field ?? g.kind)
        .join(", ")
    : "none listed";
  return supported([
    claim(
      `Coverage in the pack is cited only as recorded percentages. Listed gaps (not independently citable): ${gapText}.`,
      cov.map((row) => row.id),
    ),
  ]);
}
