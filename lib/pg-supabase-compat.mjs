import { pgQuery, pgMaybeOne, postgresConfigured } from "./postgres-node.mjs";
  ...
  async execute(mode){
    try{
      ...
      if(this.op==="select"){
        ...
        if(this.op==="upsert"&&this.conflict.length){
          this.conflict.forEach(quote);
          const target=this.conflict.map(quote).join(",");
          if(this.ignoreDuplicates)sql+=` on conflict(${target}) do nothing`;
          else{
            const mutable=keys.filter(k=>!this.conflict.includes(k));
            sql+=mutable.length
        ...
        const set=keys.map(key=>{values.push(this.patch[key]);return quote(key)+"=$"+values.length;}).join(",");
