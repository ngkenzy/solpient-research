...
function snapshotCompleteness(row){
  const values=[
    row?.revenue,row?.net_income,row?.operating_cash_flow,row?.capital_expenditure,
    row?.free_cash_flow,row?.shares_outstanding,row?.eps_diluted,
    row?.raw_payload?.balance_sheet?.cashAndCashEquivalents,
    row?.raw_payload?.balance_sheet?.totalDebt,
  ];
...
function preferSnapshot(candidate,current){
  const filedDiff=snapshotTime(candidate?.filed_at)-snapshotTime(current?.filed_at);
  if(filedDiff!==0)return filedDiff>0;
  const completenessDiff=snapshotCompleteness(candidate)-snapshotCompleteness(current);
  if(completenessDiff!==0)return completenessDiff>0;
  const fiscalDiff=Number(candidate?.fiscal_year??0)-Number(current?.fiscal_year??0);
  if(fiscalDiff!==0)return fiscalDiff>0;
  const observedDiff=snapshotTime(candidate?.observed_at)-snapshotTime(current?.observed_at);
  if(observedDiff!==0)return observedDiff>0;
  return JSON.stringify(candidate).localeCompare(JSON.stringify(current))>0;
}

export function dedupeFundamentalSnapshots(rows=[]){
  const byKey=new Map();
  let duplicateRowsRemoved=0;
  for(const row of rows){
    const key=[row?.company_id,row?.period_end,row?.form,row?.provider].join("|");
    ...
    if(preferSnapshot(row,current))byKey.set(key,row);
  }
  return{rows:[...byKey.values()],duplicateRowsRemoved};
}
