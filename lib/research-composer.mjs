...
function latestMetric(payload, key) {
  ...
  const row = rows.find((r) => (r.module ?? "universal") === "universal" && r.metric_key === key && r.status === "available");
...
function horizonStats(rows, asOfDate, years) {
  ...
  const values = eligible.map((r) => n(r[key])).filter((x) => x != null && x > 0 && x < 200);
  return { years, metric: key, median: median(values), observations: values.length };
}
...
function choosePerShare(payload) {
...
function scenarioValues({ perShare, history, peerMedian, growthBase }) {
  if (perShare == null) return { bear: null, base: null, bull: null, method: null };
  const historyMeds = history.map((h) => h.median).filter((x) => x != null);
...
function createReturnScenarios(currentPrice, values) {
  const out = [];
  for (const [scenario, terminal] of [["bear", values.bear], ["base", values.base], ["bull", values.bull]]) {
    for (const horizon of [3, 5, 10]) {
      out.push({
        ...
        expected_cagr: annualized(currentPrice, terminal, horizon),
        terminal_value: terminal,
        return_decomposition: {
          starting_price: currentPrice,
          ending_value: terminal,
          dividends_assumed: 0,
          note: "Price-only CAGR from reviewed fair-value scenario; dividends are excluded unless separately modeled."
        }
      });
    }
  }
...
export function composeResearchV1({ company, baselinePayload, contextPack = {}, valuationHistory = [], asOfDate = new Date().toISOString().slice(0, 10) }) {
  ...
  const marketCap = n(research.market_cap);
  ...
  const perShare = choosePerShare(payload);
  const peerMultiple = perShare.metric ? nearestPeerMedian(contextPack, perShare.metric) : null;
  const multipleCases = scenarioValues({ perShare: perShare.perShare, history: hist, peerMedian: peerMultiple, growthBase: fcfGrowth5 ?? growth5 ?? revenueGrowth });
  ...
  const bearValue = median([multipleCases.bear, dcfBear].filter((x) => x != null));
  const baseValue = median([multipleCases.base, dcfBase].filter((x) => x != null));
  const bullValue = median([multipleCases.bull, dcfBull].filter((x) => x != null));
  ...
  const historyRows = hist.reduce((acc, h) => {
    acc[h.years + "y"] = h.observations >= 40 && h.median != null
      ? { status: "available", metric: h.metric, median_multiple: h.median, observations: h.observations, explanation: "Median " + h.metric.replaceAll("_", " ") + " from stored daily valuation history." }
      : { status: "unavailable", metric: h.metric, median_multiple: h.median, observations: h.observations, explanation: "Stored valuation history is not yet deep enough for a reliable " + h.years + "-year comparison." };
    return acc;
  }, {});
  ...
  const expectedReturns = createReturnScenarios(currentPrice, { bear: bearValue, base: baseValue, bull: bullValue });

  const businessAssessment = {
    business_quality_rating: quality,
    moat_rating: moat,
  ...
  return {
    ...
    review_patch: {
      research: {
        summary: (company?.ticker ?? payload.ticker) + " automated research draft: " + quality + " quantitative business quality, " + ratingFromUpside(upside).replaceAll("_", " ") + " base valuation, and human review required before publication.",
      ...
      investment_thesis: {
        ...
        quantitative_case: "Revenue growth is " + pct(revenueGrowth) + ", operating margin is " + pct(opMargin) + ", and FCF margin is " + pct(fcfMargin) + ".",
        what_must_be_true: "The business must preserve durable growth, margins and per-share cash generation while avoiding material competitive or capital-allocation impairment.",
      ...
      financial_quality: {
        ...
        narrative: "Quantitative quality score is " + quantitativeQuality + "/100. " + trendText("Revenue", growth5) + " " + trendText("FCF/share", fcfGrowth5)
      },
      ...
      valuation_analysis: {
        methods: [
          ...
          { method: "historical_valuation", status: hist.some((h) => h.median != null) ? "applied" : "unavailable", reason: hist.some((h) => h.median != null) ? "Uses medians from stored valuation history." : "Historical valuation depth is insufficient." },
          { method: "peer_valuation", status: peerMultiple != null ? "applied" : "unavailable", reason: peerMultiple != null ? "Uses median valuation multiple from configured peers with local data." : "Peer multiple is unavailable." }
        ],
        assumptions: {
        ...
        model_warning: "Fair values are scenario estimates, not predictions."
      },
      ...
      decision_dashboard: {
        ...
        enterprise_value: n(research.enterprise_value),
        business_quality: quality,
        moat,
        financial_strength: quantitativeQuality,
        growth_outlook: growth5 ?? revenueGrowth,
      ...
      final_conclusion: {
        great_business: "Quantitative evidence supports a " + quality + " business-quality assessment, but qualitative moat and management evidence must still be reviewed.",
        valuation: "Base fair value is " + (baseValue == null ? "not yet supported" : "$" + baseValue.toFixed(2)) + ", implying " + pct(upside) + " upside/downside from the research price.",
        realistic_return: "Five-year base-case price CAGR is " + pct(expectedReturns.find((r) => r.scenario === "base" && r.horizon_years === 5)?.expected_cagr) + ", excluding dividends.",
        impairment_risks: "Primary monitored risks are growth durability, margin compression, valuation error and per-share capital allocation.",
      ...
      scores: {
        ...
        moat_score: existingBusiness.moat_rating ? (n(payload?.scores?.moat_score) ?? null) : null,
      ...
      valuations: {
        ...
        historical_multiple_value: perShare.perShare == null ? null : perShare.perShare * median(hist.map((h) => h.median).filter((x) => x != null)),
