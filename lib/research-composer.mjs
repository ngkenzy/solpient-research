...
function choosePerShare(payload,industryModule) {
  ...
  if(adjustedEps!=null&&adjustedEps>0)return{metric:"pe",perShare:adjustedEps,label:"adjusted EPS guidance midpoint",normalized:true};
...
function scenarioValues({ perShare, history, peerMedian, growthBase }) {
  ...
  const historyMeds = history.filter((h)=>h.available).map((h) => h.median).filter((x) => x != null);
...
export function composeResearchV1({ company, baselinePayload, contextPack = {}, valuationHistory = [], asOfDate = new Date().toISOString().slice(0, 10) }) {
  ...
  const pipelineLateStage=industryMetric(payload,industryModule,"pipeline_revenue_replacement");
  const pipelineEvidence=industryMetricText(payload,industryModule,"pipeline_revenue_replacement");
  ...
  const thesisVariables = [
    ...(isBiopharma?[
      ...
      },
      {
        ...
        status:"monitor",metric_key:"pipeline_revenue_replacement",review_frequency:"quarterly",
  ...
  return {
    ...
    review_patch: {
      ...
      valuations: {
        ...
        historical_multiple_value: perShare.perShare == null ? null : perShare.perShare * median(hist.filter((h)=>h.available).map((h) => h.median).filter((x) => x != null)),
