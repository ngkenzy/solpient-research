export const RESEARCH_COMPOSER_VERSION = "composer-v2";
...
export function composeResearchV1({ company, baselinePayload, contextPack = {}, valuationHistory = [], asOfDate = new Date().toISOString().slice(0, 10) }) {
  ...
  const dcfGrowthBase=isBiopharma ? (valuationGrowth ?? 5) : (fcfGrowth5 ?? revenueGrowth ?? 6);
  const dcfBear = dcfPerShare({ fcfPerShare: latestMetric(payload, "fcf_per_share"), growth: clamp(dcfGrowthBase * 0.5, -5, 6), discountRate: 11.5, terminalGrowth: 2 });
  const dcfBase = dcfPerShare({ fcfPerShare: latestMetric(payload, "fcf_per_share"), growth: clamp(dcfGrowthBase, 2, 10), discountRate: 10, terminalGrowth: 2.5 });
  const dcfBull = dcfPerShare({ fcfPerShare: latestMetric(payload, "fcf_per_share"), growth: clamp(dcfGrowthBase * 1.4, 5, 14), discountRate: 9, terminalGrowth: 3 });
    ...
    review_patch: {
      ...
      valuation_analysis: {
        ...
        valuation_bridge:{
          ...
          primary_per_share_metric:perShare.metric,
          primary_per_share_label:perShare.label,
          primary_per_share_value:perShare.perShare,
          peer_fcf_multiple:peerFcfMultiple,
          normalized_fcf_per_share:fcfPerShareForPeer,
          components:{
            bear:{
              normalized_multiple:multipleCases.bear,
              dcf:dcfBear,
              normalized_peer_fcf:normalizedPeerValue==null?null:normalizedPeerValue*0.75,
              result:bearValue
            },
            base:{
              normalized_multiple:multipleCases.base,
              dcf:dcfBase,
              normalized_peer_fcf:normalizedPeerValue,
              result:baseValue
            },
            bull:{
              normalized_multiple:multipleCases.bull,
              dcf:dcfBull,
              normalized_peer_fcf:normalizedPeerValue==null?null:normalizedPeerValue*1.25,
              result:bullValue
            }
          }
        },
