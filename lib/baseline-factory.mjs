...
import { companyEvidenceForTicker } from "./company-evidence.mjs";
import { REQUIRED_METRIC_KEYS, RECOMMENDED_METRIC_KEYS, validateResearchStandard } from "./research-standard-v1.mjs";

export const BASELINE_FACTORY_VERSION = "baseline-v2";
...
function fiscalQuarter(row) {
  const explicit=String(row?.fiscal_period??"").trim().toUpperCase();
  if(/^Q[1-4]$/.test(explicit))return explicit;

  const year=n(row?.fiscal_year);
  const dateMs=Date.parse(String(row?.period_end??"")+"T00:00:00Z");
  if(!year||!Number.isFinite(dateMs))return null;

  // Providers often differ from SEC period-end dates by a few calendar days.
  // Map unlabeled provider rows to the nearest labeled fiscal quarter in the
  // same fiscal year, then let provider priority choose the canonical row.
  const labeled=selectFundamentals._labeledRows??[];
  let best=null;
  for(const candidate of labeled){
    if(n(candidate?.fiscal_year)!==year)continue;
    const period=String(candidate?.fiscal_period??"").trim().toUpperCase();
    if(!/^Q[1-4]$/.test(period))continue;
    const candidateMs=Date.parse(String(candidate?.period_end??"")+"T00:00:00Z");
    if(!Number.isFinite(candidateMs))continue;
    const distance=Math.abs(candidateMs-dateMs);
    if(distance<=45*24*60*60*1000&&(!best||distance<best.distance)){
...
function selectFundamentals(rows = []) {
  const quarterlyRows = rows.filter((row) => String(row?.fiscal_period ?? "").toUpperCase() !== "FY");
  selectFundamentals._labeledRows=quarterlyRows.filter((row)=>/^Q[1-4]$/.test(String(row?.fiscal_period??"").trim().toUpperCase()));

  const normalized=quarterlyRows.map((row)=>({
    row,
    fiscalQuarter:fiscalQuarter(row),
    fiscalYear:n(row?.fiscal_year),
  }));

  const sorted=[...normalized].sort((a,b)=>{
    const aCanonical=Boolean(a.fiscalYear&&a.fiscalQuarter);
    const bCanonical=Boolean(b.fiscalYear&&b.fiscalQuarter);
    if(aCanonical&&bCanonical&&a.fiscalYear===b.fiscalYear&&a.fiscalQuarter===b.fiscalQuarter){
      const provider=(PROVIDER_PRIORITY[a.row.provider]??8)-(PROVIDER_PRIORITY[b.row.provider]??8);
      if(provider!==0)return provider;
    }
    const byDate=String(b.row.period_end??"").localeCompare(String(a.row.period_end??""));
    if(byDate!==0)return byDate;
    return (PROVIDER_PRIORITY[a.row.provider]??8)-(PROVIDER_PRIORITY[b.row.provider]??8);
  });
  ...
  for(const entry of sorted){
    const key=entry.fiscalYear&&entry.fiscalQuarter
      ?entry.fiscalYear+"|"+entry.fiscalQuarter
      :"date|"+String(entry.row.period_end??"");
    if(seen.has(key))continue;
    seen.add(key);
    selected.push(entry.row);
  }
  selectFundamentals._labeledRows=[];
  return selected;
}
...
function comparablePrior(rows, latest) {
  ...
  const priorYear=n(latest.fiscal_year)-1;
  const period=String(latest.fiscal_period ?? "").trim().toUpperCase();
  if (period && period!=="FY") {
    return rows.find((row) =>
      row.period_end !== latest.period_end &&
      String(row.fiscal_period ?? "").trim().toUpperCase()===period &&
      n(row.fiscal_year)===priorYear
    ) ?? null;
  }

  const latestDate=new Date(String(latest.period_end)+"T00:00:00Z");
  ...
  const candidates=rows
    .filter((row)=>
      row.period_end!==latest.period_end &&
      n(row.fiscal_year)===priorYear &&
      String(row.fiscal_period ?? "").trim().toUpperCase()!=="FY"
    )
    .map((row)=>({
      row,
      distance:Math.abs(new Date(String(row.period_end)+"T00:00:00Z").getTime()-expected.getTime()),
    }))
  ...
  return best && best.distance<=maxDistance ? best.row : null;
}
...
function moduleObservation(module, key, context) {
  ...
  const definitions = {
    ...
    payout_ratio: () => observation({
      module, metricKey:key, label:"Dividend payout ratio",
      value: lookup("payout_ratio"), unit:"percent",
      periodEnd:latest?.period_end, periodType:"ttm",
      source:latest, calculationMethod:"Absolute TTM common dividends paid / TTM net income.",
    }),
    net_debt_to_fcf: () => observation({
      module, metricKey:key, label:"Net debt / FCF",
      value: ratio(lookup("net_debt"), lookup("free_cash_flow")),
      unit:"x", periodEnd:latest?.period_end, periodType:"ttm",
      source:latest, calculationMethod:"Net debt / TTM free cash flow.",
    }),
...
export function buildBaselineDraft({
  ...
  const module = industryModuleOverride ?? industryModuleForTicker(ticker);
  ...
  const companyEvidence=companyEvidenceForTicker(ticker);
  if (Array.isArray(companyEvidence.metricObservations) && companyEvidence.metricObservations.length) {
    const evidenceMap=new Map(companyEvidence.metricObservations.map((row)=>[(row.module??module)+":"+row.metric_key,row]));
    moduleRows=moduleRows.map((row)=>evidenceMap.get(row.module+":"+row.metric_key) ?? row);
    for (const row of companyEvidence.metricObservations) {
      const key=(row.module??module)+":"+row.metric_key;
      if (!moduleRows.some((existing)=>(existing.module+":"+existing.metric_key)===key)) moduleRows.push(row);
    }
  }

  const requiredUniversalRows = REQUIRED_METRIC_KEYS.map((key) => universalByKey.get(key)).filter(Boolean);
  const requiredModuleRows = definition
    ? definition.requiredMetricKeys.map((key)=>moduleRows.find((row)=>row.module===module&&row.metric_key===key)).filter(Boolean)
    : [];
  ...
  for (const source of companyEvidence.sources ?? []) {
    if (!source?.url || sources.some((existing)=>existing.url===source.url)) continue;
    sources.push(source);
  }
  const timestamps = [
    market?.observed_at,
  ...
  const payload = {
    ...
    business_assessment: {
      ...
      evidence: [],
      ...(companyEvidence.businessAssessment ?? {}),
    },
