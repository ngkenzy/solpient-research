...
const PROVIDER_PRIORITY = { fmp: 0, alpha_vantage: 1, unknown: 9 };
...
function valueFromRaw(row, section, key) {
  const source = section === "income" ? rawIncome(row) : rawCash(row);
  return n(source?.[key]);
}
...
function selectFundamentals(rows = []) {
  ...
  return sorted.filter((row) => {
    const key = [row.period_end, row.fiscal_period ?? "", row.form ?? ""].join("|");
...
function ttm(rows) {
  ...
  const sum = (key) => quarters.reduce((acc, row) => {
    const value = n(row[key]);
    return value == null ? acc : acc + value;
  }, 0);
  const rawSum = (section, key) => quarters.reduce((acc, row) => {
    const value = valueFromRaw(row, section, key);
    return value == null ? acc : acc + value;
  }, 0);
  ...
  return {
    ...
    capex: quarters.reduce((acc, row) => acc + Math.abs(n(row.capital_expenditure) ?? 0), 0),
...
export function buildBaselineDraft({
  ...
  const latestCash = valueFromRaw(latest, "cash", "cashAtEndOfPeriod");
  const currentShares = n(latest?.shares_outstanding);
  ...
  const universal = [
    ...
    unavailable("universal","total_debt","Total debt","Balance-sheet debt is not currently normalized in the automated provider feed."),
    unavailable("universal","net_debt","Net debt","Requires normalized balance-sheet debt and cash."),
    observation({
    ...
    unavailable("universal","enterprise_value","Enterprise value","Requires normalized total debt before enterprise value is defensible."),
    ...
    unavailable("universal","share_count_cagr_5y","Share-count CAGR 5Y","Five-year diluted-share history is not yet present in the automated snapshot store."),
    observation({
