...
function fiscalQuarter(row,labeledRows=[]) {
  ...
  let best=null;
  for(const candidate of labeledRows){
...
function preferFundamentalRow(candidate,current){
  if(!current)return candidate;
  const candidatePriority=PROVIDER_PRIORITY[candidate.provider]??8;
  const currentPriority=PROVIDER_PRIORITY[current.provider]??8;
  if(candidatePriority!==currentPriority){
    return candidatePriority<currentPriority?candidate:current;
  }
  return String(candidate.period_end??"")>String(current.period_end??"")?candidate:current;
}

function selectFundamentals(rows = []) {
  const quarterlyRows=rows.filter((row)=>String(row?.fiscal_period??"").toUpperCase()!=="FY");
  const labeledRows=quarterlyRows.filter((row)=>/^Q[1-4]$/.test(String(row?.fiscal_period??"").trim().toUpperCase()));
  const groups=new Map();

  for(const row of quarterlyRows){
    const fiscalYear=n(row?.fiscal_year);
    const quarter=fiscalQuarter(row,labeledRows);
    const key=fiscalYear&&quarter
      ?fiscalYear+"|"+quarter
      :"date|"+String(row?.period_end??"");
    groups.set(key,preferFundamentalRow(row,groups.get(key)));
  }
