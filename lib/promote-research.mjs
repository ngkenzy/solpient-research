...
export function buildPublicationPackage(payload, promotedAt = new Date().toISOString()) {
  ...
  return {
    ...
    sources:normalizeSourceRows(publicationPayload.sources ?? [],promotedAt).map(cleanRow),
...
export function buildPublicationIntegrity({draft,review,composition,packagePayload,readiness}) {
  const evidencePackage={
    source_cutoff_at:draft?.source_cutoff_at ?? null,
    evidence_summary:draft?.evidence_summary ?? {},
    metric_observations:packagePayload.metric_observations,
    sources:packagePayload.sources,
  };
  const normalizedInputs={
    ...
    fundamental_scorecard:packagePayload.fundamental_scorecard,
  ...
  return {
    ...
    published_output_hash:canonicalSha256(packagePayload),
...
export async function promoteReviewedBaseline({ supabase, draft, review, payload, promotedAt = new Date().toISOString() }) {
  ...
  const integrity=buildPublicationIntegrity({
    draft,review,composition,packagePayload,readiness,
  });
