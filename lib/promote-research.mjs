...
import { validateHumanReviewAttestation } from "./review-attestation.mjs";
import { buildResearchInputManifest, stageResearchInputManifest } from "./evidence-provenance-db.mjs";
import { postgresConfigured, pgQuery, pgMaybeOne } from "./postgres-node.mjs";
import { buildResearchInputManifestPg, stageResearchInputManifestPg } from "./evidence-provenance-pg.mjs";
...
export function buildPublicationIntegrity({draft,review,composition,packagePayload,readiness,manifest=null}) {
  ...
  return {
    ...
    human_review_attestation:{
      version:review?.attestation_version ?? null,
      verified_at:review?.human_verified_at ?? null,
      verified_by:review?.human_verified_by ?? null,
      payload_hash:review?.human_verified_payload_hash ?? null,
    },
...
async function promoteReviewedBaselinePostgres({draft,review,payload,promotedAt,readiness}) {
  const ingestionKey="baseline-draft:"+draft.id;
  const existing=await pgMaybeOne(
    `select id,version from public.research_runs where ingestion_key=$1 limit 1`,
    [ingestionKey],
  );
  ...
  const [latestRun,composition]=await Promise.all([
    pgMaybeOne(
    ...
    pgMaybeOne(
      `select *
       ...
       where draft_id=$1 and status='applied'
  ...
  if(latestRun){
    [previousMetrics,previousScores,previousValuation,previousThesis]=await Promise.all([
      pgMaybeOne(`select * from public.financial_metrics where research_run_id=$1 limit 1`,[latestRun.id]),
      pgMaybeOne(`select * from public.scores where research_run_id=$1 limit 1`,[latestRun.id]),
      pgMaybeOne(`select * from public.valuations where research_run_id=$1 limit 1`,[latestRun.id]),
      pgQuery(`select * from public.thesis_variables where research_run_id=$1`,[latestRun.id]),
    ]);
  }
  ...
  const manifest=await buildResearchInputManifestPg({
    draft,composition,packagePayload,
  });
  await stageResearchInputManifestPg(manifest);
  ...
  const row=await pgMaybeOne(
    `select public.publish_reviewed_research_v2(
       $1::uuid,$2::uuid,$3::uuid,$4::uuid,$5::jsonb,$6::jsonb,$7::jsonb
     ) as result`,
    [
      draft.id,
      review.id,
...
export async function promoteReviewedBaseline({ supabase, draft, review, payload, promotedAt = new Date().toISOString() }) {
  ...
  const attestation=validateHumanReviewAttestation({draft,review,payload});
  if (!attestation.valid) {
    throw new Error("Promotion blocked: explicit human verification is required for the exact current review payload. "+attestation.reasons.join(" "));
  }

  if(postgresConfigured()){
    return promoteReviewedBaselinePostgres({draft,review,payload,promotedAt,readiness});
  }
  if(!supabase)throw new Error("No publication data source is configured.");
