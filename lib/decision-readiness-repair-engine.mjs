...
function instructionFor(key,target,input={}){
  ...
  if(key==="valuation_history_pct"){
    if(target>=60)return "Extend point-in-time valuation history to at least 60% coverage; the canonical minimum is 60 valuation observations spanning at least 1.4 years.";
    return `Raise point-in-time valuation history coverage to at least ${target}%.`;
  }
...
function boosterActions(state,targetState){
  ...
  for(const [key,target] of preferred){
