...
const COVERAGE_STATUS_PRIORITY = {
  pending: 0,
  unavailable: 1,
  partial: 2,
  verified_none: 5,
...
export function shouldReplaceCoverage(existing, next) {
  if (!existing) return true;
  const existingStatus = COVERAGE_STATUS_PRIORITY[existing.status] ?? 0;
  const nextStatus = COVERAGE_STATUS_PRIORITY[next.status] ?? 0;
  if (nextStatus !== existingStatus) return nextStatus > existingStatus;

  const existingProvider = PROVIDER_PRIORITY[existing.provider] ?? 0;
  const nextProvider = PROVIDER_PRIORITY[next.provider] ?? 0;
  if (nextProvider !== existingProvider) return nextProvider > existingProvider;

  const existingVerified = Date.parse(existing.verified_at ?? "") || 0;
  const nextVerified = Date.parse(next.verified_at ?? "") || 0;
  return nextVerified >= existingVerified;
}
