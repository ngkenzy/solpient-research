...
export function normalizeMarketBeatInsiders({
  ...
  const verifiedNone=rows.length===0&&buys===0&&sells===0;
  ...
  return {
    ...
    coverage:{
      company_id:company.id,
      activity_type:"insider",
      status:rows.length?"activity_found":verifiedNone?"verified_none":"partial",
      provider:MARKETBEAT_CAPITAL_PROVIDER,
      window_start:monthsAgo(end,12),
      window_end:end,
      verified_at:verifiedAt,
      ...
      notes:rows.length
        ?"MarketBeat public insider-trades page returned qualifying Buy/Sell rows."
        :verifiedNone
      ...
      metadata:{source:"public_html",summary_buys:buys,summary_sells:sells},
...
export function normalizeMarketBeatInstitutional({
  ...
  return {
    ...
    coverage:{
      company_id:company.id,
      activity_type:"institutional",
      status:rows.length?"activity_found":unavailable?"unavailable":"partial",
      provider:MARKETBEAT_CAPITAL_PROVIDER,
      window_start:null,
      window_end:end,
      verified_at:verifiedAt,
