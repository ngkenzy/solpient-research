...
export async function runBaselineResearchBatch({
  ...
  for (const row of rows) {
    ...
    try {
      ...
      const persistence = persist
        ? await persistFn({ pack, output, validation })
      ...
      if (persistence.persisted) {
        ...
        summary.persistence_skipped_count += 1;
      }
      ...
      summary.results.push({
        ticker,
        status: persist && !persistence.persisted ? "persistence_failed" : "success",
        evidence_pack_hash: pack.evidence_pack_hash,
        research_state: state,
  ...
  summary.authoritative =
    ...
    summary.persistence_failed_count === 0 &&
