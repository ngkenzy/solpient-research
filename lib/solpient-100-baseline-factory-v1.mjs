...
export function deriveSolpient100BaselinePlan(
  ...
  const publishedAt=ms(state.published_research_researched_at);
  const latestDurableEvidenceAt=newest(
    state.latest_fundamental_observed_at,
    state.latest_material_filing_at,
    state.industry_assignment_created_at,
  );
  const publishedIsCurrent=
    Boolean(state.published_research_run_id) &&
    publishedAt!=null &&
    (latestDurableEvidenceAt==null || latestDurableEvidenceAt<=publishedAt);
  ...
  if (publishedIsCurrent) {
    return {
      ...
      steps,
      needs_composition: false,
      reason: "published_research_current",
    };
  }

  if (state.published_research_run_id && !publishedIsCurrent) {
    warnings.push("published_research_has_newer_durable_evidence");
  }
  ...
  // Daily market prices change the opportunity score, but they do not by
  // themselves invalidate qualitative business analysis. Rebuild research
  // context only when durable company evidence changes.
  ...
  const reviewCurrent =
    Boolean(state.review_id) &&
    Boolean(state.baseline_draft_id) &&
    state.review_draft_id === state.baseline_draft_id;
  ...
  const needsComposition = !reviewCurrent && (baselineStale || compositionInvalid);
  if (needsComposition) {
    steps.push("compose_review_package");
  }
  if (!reviewCurrent) {
    steps.push("prepare_review_package");
  }
  ...
  return {
    ticker,
    status: complete
      ? state.review_promotion_ready
        ? state.review_human_verified_at
    ...
    current: {
      ...
      review_status: state.review_status ?? null,
      review_promotion_ready: state.review_promotion_ready === true,
      review_human_verified: Boolean(state.review_human_verified_at),
    },
  };
}
...
export function summarizeSolpient100BaselineStates(states = []) {
  ...
  for (const state of states) {
    ...
    else if (["verified_for_release","ready_for_verification"].includes(plan.status)) summary.baseline_ready_count += 1;
    else if (plan.status === "staged_for_review") summary.baseline_building_count += 1;
