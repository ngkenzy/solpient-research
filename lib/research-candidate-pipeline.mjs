...
export const RESEARCH_CANDIDATE_PIPELINE_VERSION="research-candidate-pipeline-v2.3";
...
function nextActions({eligible,companyExists,preflight,valuation,decision,sectorEvidence,sectorClassification}){
  const actions=[];
  if(!eligible){
    return [{
      priority:100,
  ...
  if(sectorEvidence?.missingCriticalEvidence?.length){
    actions.push({
      priority:98,
      type:"sector_evidence",
      action:"Collect missing sector-critical evidence.",
      reason:"Sector Evidence Model V2.1 is capping effective screening coverage until decision-critical industry evidence is present.",
      currentCoverage:sectorEvidence.effectiveCoveragePct??null,
      evidenceCeiling:sectorEvidence.evidenceCoverageCeilingPct??null,
      missing:sectorEvidence.missingCriticalEvidence.map(x=>x.label??x.key),
    });
  }
  if(sectorClassification?.reviewRequired){
    actions.push({
      priority:100,
      type:"classification_review",
      action:"Complete reviewed sector classification before Solpient 100 candidate promotion.",
      reason:sectorClassification.reviewReason??sectorEvidence?.blocker??"Sector classification requires human review.",
    });
  }else if(sectorEvidence?.candidatePromotionBlocked&&sectorEvidence?.blocker){
    actions.push({
      priority:99,
      type:"classification",
      action:"Repair sector classification before Solpient 100 candidate promotion.",
      reason:sectorEvidence.blocker,
    });
  }
...
export function buildResearchCandidatePipeline({
  ...
  const sectorEvidence=screenResult.sectorEvidence??screenResult.sector_evidence??null;
  const sectorClassification=screenResult.sectorClassification??screenResult.sector_classification??null;
  const actions=nextActions({
    eligible,companyExists,preflight,valuation:valuation??{},decision,sectorEvidence,sectorClassification,
  });
  ...
  return {
    ...
    screening:{
      ...
      rawEvidenceCoveragePct:n(screenResult.rawEvidenceCoveragePct??screenResult.raw_evidence_coverage_pct),
    ...
    safeguards:[
      ...
      "Sector Evidence Model V2.1 blockers remain explicit research tasks and are not converted into neutral scores.",
      "Sector Classification Coverage V2.3 review-required names stay blocked until classification review is resolved.",
    ],
  };
}
