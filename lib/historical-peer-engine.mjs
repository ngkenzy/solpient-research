...
function ttmAt(fundamentals,dateValue){
  const eligible=latestByDate(fundamentals.filter(r=>String(r.fiscal_period??"").toUpperCase()!=="FY"&&String(r.period_end)<=String(dateValue))).slice(0,4);
...
export function buildCompanyHistory({company,fundamentals=[],markets=[]}){
  ...
  const valuations=valuationRows(company,module,quarterRows,markets);
