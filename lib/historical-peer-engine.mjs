...
function aggregateYear(companyId,yearRows){
  ...
  const netDebtIssuance=sum(r=>raw(r,"cash","netDebtIssuance")),debtIssued=netDebtIssuance!=null&&netDebtIssuance>0?netDebtIssuance:null,debtRepaid=netDebtIssuance!=null&&netDebtIssuance<0?Math.abs(netDebtIssuance):null;
  ...
  return [
    ...
    metricRow(companyId,base,"debt_issued","Debt issued",debtIssued,"USD","derived","fiscal_year"),
    metricRow(companyId,base,"debt_repaid","Debt repaid",debtRepaid,"USD","derived","fiscal_year"),
  ].filter(x=>x.value_numeric!==null);
}
