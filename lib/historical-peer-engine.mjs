...
export const CONTEXT_ENGINE_VERSION="context-v1";
...
function aggregateYear(companyId,yearRows){
  const rows=latestByDate(yearRows);
  if(rows.length<4)return [];
  const q=rows.slice(0,4);
  const sum=(getter)=>q.reduce((a,r)=>a+(getter(r)??0),0);
...
function ttmAt(fundamentals,dateValue){
  const eligible=latestByDate(fundamentals.filter(r=>String(r.period_end)<=String(dateValue))).slice(0,4);
  if(eligible.length<4)return null;
  const sum=k=>eligible.reduce((a,r)=>a+(n(r[k])??0),0);
  return {fcf:sum("free_cash_flow"),netIncome:sum("net_income"),revenue:sum("revenue")};
}
...
function ttmCapital(capital){
  const q=latestByDate(capital).slice(0,4),sum=k=>q.reduce((a,r)=>a+(n(r[k])??0),0);
  if(!q.length)return {};
...
export function buildCompanyHistory({company,fundamentals=[],markets=[]}){
  const module=industryModuleForTicker(company.ticker),ordered=latestByDate(fundamentals);
  const quarterly=ordered.flatMap(row=>quarterlyMetrics(company.id,row));
  ...
  const capital=capitalRows(company.id,ordered);
...
export function buildPeerContext({company,trackedCompanies,latestByTicker,asOfDate}){
  ...
  for(const peer of peerSet){
    ...
    for(const key of ["revenue_growth_yoy","gross_margin","operating_margin","net_margin","fcf_margin","fcf_per_share_growth_yoy","share_count_growth_yoy","pe","price_to_fcf","fcf_yield"]){
      ...
      snapshotRows.push({company_id:company.id,peer_ticker:peer.ticker,metric_key:key,as_of_date:asOfDate,value_numeric:metrics[key],value_text:null,unit:key.includes("margin")||key.includes("growth")||key==="fcf_yield"?"percent":key==="pe"||key==="price_to_fcf"?"x":null,provider:"solpient-context-v1",source_url:null,observed_at:new Date().toISOString()});
    }
...
export function buildContextPack({company,result,peerContext,asOfDate}){
  return {
    company_id:company.id,context_version:CONTEXT_ENGINE_VERSION,as_of_date:asOfDate,generated_at:new Date().toISOString(),industry_module:result.module,
