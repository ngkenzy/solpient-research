...
function selectFundamentals(rows=[]){
  const sorted=[...rows].sort((a,b)=>{
    ...
    return byDate||((PROVIDER_PRIORITY[a.provider]??8)-(PROVIDER_PRIORITY[b.provider]??8));
  });
...
function ttmAt(fundamentals,dateValue){
  ...
  const sum=k=>{
    const values=eligible.map((r)=>n(r[k]));
    return values.some((value)=>value==null) ? null : values.reduce((a,value)=>a+value,0);
  };
...
export function buildCompanyHistory({company,fundamentals=[],markets=[]}){
  const module=industryModuleForTicker(company.ticker),ordered=selectFundamentals(fundamentals);
