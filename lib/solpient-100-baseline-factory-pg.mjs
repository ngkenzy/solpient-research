...
export async function loadLatestSolpient100BaselineStates({
  ...
  const [
    ...
    ? await Promise.all([
        ...
        pgQuery(
          `select
             company_id,
             count(*)::int as material_filing_count,
             max(coalesce(accepted_at,filed_at::timestamptz)) as latest_material_filing_at,
             max(filed_at) as latest_material_filing_date
           from public.filing_events
           where company_id=any($1::uuid[])
             and provider='sec_submissions'
             and upper(replace(form_type,'/A','')) in ('10-K','10-Q','8-K')
           group by company_id`,
          [companyIds],
        ),
        pgQuery(
  ...
  const filingByCompany = mapByCompany(filingStats);
  ...
  const states = members.map((member, index) => {
    ...
    const fundamentals = companyId
      ...
      : null;
    const filing = companyId ? filingByCompany.get(companyId) ?? null : null;
    ...
    return {
      ...
      material_filing_count: Number(filing?.material_filing_count ?? 0),
      latest_material_filing_at: filing?.latest_material_filing_at ?? null,
      latest_material_filing_date: filing?.latest_material_filing_date ?? null,
      ...
      published_research_researched_at: research?.researched_at ?? null,
    };
  });
