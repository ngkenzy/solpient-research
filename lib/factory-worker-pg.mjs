...
export async function createResearchFactoryRunPg(runPayload,items){
  ...
  if(existing){
    if(existing.input_hash!==runPayload.input_hash){
      throw new Error(
        "Existing Research Factory run for source pipeline "+
        runPayload.source_pipeline_run_id+
        " and version "+runPayload.factory_version+
        " has input_hash "+existing.input_hash+
        ", but current materialization computed "+runPayload.input_hash+
        ". The source pipeline should be immutable; investigate before replacing the run."
      );
    }
    return existing.id;
  }
  ...
  try{
    ...
    if(error?.code==="23505"){
      ...
      if(raced&&raced.input_hash===runPayload.input_hash)return raced.id;
    }
    throw error;
  }
}
