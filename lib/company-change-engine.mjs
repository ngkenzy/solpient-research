import {mapLegacyCategory,TAXONOMY_VERSION} from "./event-detectors.mjs";

export const COMPANY_STATE_VERSION="company-state-v1";
// V1 event ingestion: snapshot-diff events use the PRD §15 taxonomy. Legacy
// engine categories are mapped at emit time so the FK on
// company_change_events.category always holds.
...
function addNumeric(events,{
  ...
  events.push({
    ...
    research_run_id:researchRunId,event_key:"metric:"+metricKey,category:mapLegacyCategory(category),metric_key:metricKey,label,event_taxonomy:TAXONOMY_VERSION,
...
export function buildCompanyChangeEvents({
  ...
  for(const [key,item] of currThesis){
    ...
    events.push({
      company_id:companyId,current_snapshot_id:currentSnapshotId,previous_snapshot_id:previousSnapshotId,
      research_run_id:researchRunId,event_key:"thesis:"+key,category:mapLegacyCategory("thesis"),metric_key:"thesis:"+key,event_taxonomy:TAXONOMY_VERSION,
  ...
  if(currFiling?.accession_number&&currFiling.accession_number!==prevFiling?.accession_number){
    events.push({
      company_id:companyId,current_snapshot_id:currentSnapshotId,previous_snapshot_id:previousSnapshotId,
      research_run_id:researchRunId,event_key:"filing:"+currFiling.accession_number,category:mapLegacyCategory("filing"),metric_key:"latest_filing",event_taxonomy:TAXONOMY_VERSION,
  ...
  if(current.research?.version!=null&&previous.research?.version!=null&&current.research.version!==previous.research.version){
    events.push({
      company_id:companyId,current_snapshot_id:currentSnapshotId,previous_snapshot_id:previousSnapshotId,
      research_run_id:researchRunId,event_key:"research-version:"+current.research.version,category:mapLegacyCategory("research"),metric_key:"research_version",event_taxonomy:TAXONOMY_VERSION,
