...
export const METHODOLOGY_ACTIVATION_VERSION="methodology-validation-activation-v1.1";
export const DEFAULT_FULL_UNIVERSE_MIN_INPUT_COUNT=1000;
...
export function canonicalUniverseRows(rows=[]){
  return rows.map(row=>({
    ...row,
...
export function universeInputHash(rows=[]){
  return canonicalSha256({
    contract:"solpient-full-universe-input-v1",
    rows:canonicalUniverseRows(rows),
  });
}

function isGovernedSuccessor(definitions=[],candidate,requiredVersion){
  if(!candidate||candidate.version===requiredVersion)return Boolean(candidate);
  const byIdentity=new Map(definitions.map(d=>[d.methodology_key+"|"+d.version,d]));
  const visited=new Set();
  let cursor=candidate;
  while(cursor?.predecessor_version||cursor?.manifest?.predecessor_version){
    const predecessorVersion=cursor.predecessor_version??cursor.manifest?.predecessor_version;
    const identity=cursor.methodology_key+"|"+predecessorVersion;
    if(visited.has(identity))return false;
    visited.add(identity);
    if(predecessorVersion===requiredVersion)return true;
    cursor=byIdentity.get(identity)??null;
  }
  return false;
}

export function methodologyDependencyStatus(definitions=[],events=[],manifest={},options={}){
  const dependencies=(manifest?.dependencies??[]).filter(d=>d?.required!==false);
  const activationIdentities=options.activationIdentities instanceof Set
    ?options.activationIdentities
    :new Set(options.activationIdentities??[]);
  const rows=dependencies.map(dependency=>{
    const exact=definitions.find(d=>
      d.methodology_key===dependency.methodology_key&&
      d.version===dependency.version
    )??null;
    const lifecycle=exact
      ?deriveLifecycle(events.filter(e=>e.methodology_definition_id===exact.id))
      :null;
    const sameKeyPredecessor=dependency.methodology_key===manifest.methodology_key;
    const identity=dependency.methodology_key+"|"+dependency.version;
    const inAtomicActivationSet=activationIdentities.has(identity);
    const targetReady=inAtomicActivationSet&&["validated","active"].includes(lifecycle);
    const governedSuccessor=definitions.find(candidate=>{
      const candidateIdentity=candidate.methodology_key+"|"+candidate.version;
      if(candidate.methodology_key!==dependency.methodology_key)return false;
      if(!activationIdentities.has(candidateIdentity))return false;
      const candidateLifecycle=deriveLifecycle(
        events.filter(e=>e.methodology_definition_id===candidate.id)
      );
      if(!["validated","active"].includes(candidateLifecycle))return false;
      return isGovernedSuccessor(definitions,candidate,dependency.version);
    })??null;
    return{
      methodology_key:dependency.methodology_key,
      version:dependency.version,
      registered:Boolean(exact),
      lifecycle_state:lifecycle,
      same_key_predecessor:sameKeyPredecessor,
      in_atomic_activation_set:inAtomicActivationSet,
      governed_successor_version:governedSuccessor?.version??null,
      satisfied:Boolean(exact)&&(
        sameKeyPredecessor||
        targetReady||
        lifecycle==="active"||
        Boolean(governedSuccessor)
      ),
    };
  });
  return{
    ready:rows.every(x=>x.satisfied),
    rows,
    missing:rows.filter(x=>!x.registered),
    inactive:rows.filter(x=>x.registered&&!x.satisfied),
  };
}
...
export function buildMethodologyValidationBundle(rows=[],options={}){
  ...
  const minInputCount=Math.max(1,Number(options.minInputCount??DEFAULT_FULL_UNIVERSE_MIN_INPUT_COUNT)||DEFAULT_FULL_UNIVERSE_MIN_INPUT_COUNT);
  const canonicalRows=canonicalUniverseRows(rows);
  const universe_input_hash=universeInputHash(canonicalRows);
  const qa=buildUniverseQAReport(canonicalRows,{limit,thresholds:options.qaThresholds??{}});
  const classification=auditClassificationCoverage(canonicalRows);
  const acceptance={
    ...
    full_universe_size:canonicalRows.length>=minInputCount,
  };
  ...
  if(!acceptance.full_universe_size){
    blockingReasons.push(
      "Input count "+canonicalRows.length+" is below the required validation minimum "+minInputCount+"."
    );
  }
  ...
  const payload={
    ...
    input_count:canonicalRows.length,
...
export function methodologyStackStatus(definitions=[],events=[],stack=UNIVERSE_METHOD_STACK){
  ...
  const rows=stack.map(spec=>{
    ...
    const definitionEvents=def?(eventsByDef.get(def.id)??[]):[];
    const lifecycle=def?deriveLifecycle(definitionEvents):null;
    const activeEvent=[...definitionEvents]
      .filter(e=>e.event_type==="active")
      .sort((a,b)=>String(b.effective_at??b.created_at??"").localeCompare(String(a.effective_at??a.created_at??"")))[0]??null;
    return{
      ...spec,
