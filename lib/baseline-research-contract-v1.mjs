...
const FINANCIAL_UNIVERSAL_EXCLUSIONS = [
  "gross_margin",
  "operating_cash_flow",
  "free_cash_flow",
  "fcf_margin",
...
const UNIVERSAL_METRIC_EXCLUSIONS_BY_MODULE = Object.freeze({
  financial_bank: new Set(["operating_margin", ...FINANCIAL_UNIVERSAL_EXCLUSIONS]),
  financial_insurance: new Set(FINANCIAL_UNIVERSAL_EXCLUSIONS),
  financial_services: new Set(FINANCIAL_UNIVERSAL_EXCLUSIONS),
  real_estate_reit: new Set([
    "free_cash_flow",
    "fcf_margin",
    "fcf_per_share",
    "fcf_conversion",
...
function requiredUniversalMetricKeys(module) {
  const exclusions = UNIVERSAL_METRIC_EXCLUSIONS_BY_MODULE[module] ?? new Set();
  return V2_CORE_METRIC_KEYS.filter((key) => !exclusions.has(key));
}
...
function identityEvidence(company = {}) {
  return {
    id: "identity:company",
    kind: "company_identity",
    label: company.company_name ?? upper(company.ticker),
    ...
    source: { table: "companies", company_id: company.id ?? null },
  };
}

function sectorAssignmentEvidence({
  module,
  proposedModule,
  industryAssignment,
  assignmentApplied,
  explicitReviewedModule,
} = {}) {
  if (!module && !proposedModule) return null;
  return {
    id: "sector:assignment",
    kind: "sector_assignment",
    label: module ?? proposedModule,
    module: module ?? null,
    proposed_module: proposedModule ?? null,
    reviewed: Boolean(module && INDUSTRY_MODULES[module]),
    status: assignmentApplied ? "applied" : explicitReviewedModule ? "reviewed_input" : "unreviewed",
    policy_version: industryAssignment?.policy_version ?? null,
    confidence: numberOrNull(industryAssignment?.confidence),
    decision_hash: industryAssignment?.decision_hash ?? null,
    source: assignmentApplied
      ? { table: "research_factory_industry_assignments", id: industryAssignment?.id ?? null }
      : explicitReviewedModule
        ? { type: "explicit_reviewed_input" }
        : { type: "proposal_only" },
  };
}

function gapEvidence(gaps = []) {
  return gaps.map((gap, index) => {
    const discriminator =
      ...
      String(index + 1);
    return {
      id: `gap:${stableEvidenceToken(gap.kind ?? "unknown")}:${stableEvidenceToken(discriminator) || index + 1}`,
      kind: "evidence_gap",
      label: gap.message ?? gap.metric_key ?? gap.field ?? gap.kind ?? "Evidence gap",
      gap_kind: gap.kind ?? null,
      module: gap.module ?? null,
      metric_key: gap.metric_key ?? null,
...
export function buildBaselineEvidencePack({
  ...
  const assignmentApplied =
    industryAssignment?.status === "applied" &&
    Number(industryAssignment?.confidence ?? 0) >= 0.8 &&
    Boolean(industryAssignment?.module && INDUSTRY_MODULES[industryAssignment.module]);
  const explicitReviewedModule =
    Boolean(industryModule && INDUSTRY_MODULES[industryModule]) ? industryModule : null;
  const module = assignmentApplied
    ? industryAssignment.module
    : explicitReviewedModule;
  const proposedModule =
    module ??
    industryAssignment?.proposed_module ??
  ...
  const universalRequiredKeys = requiredUniversalMetricKeys(module);
  const universalRequired = universalRequiredKeys.length;
  const universalCovered = universalRequiredKeys.filter((key) => universalAvailable.has(key)).length;
  ...
  const substantiveEvidenceItems = [
    ...metricEvidence,
    ...documentEvidence,
    ...valuationRows,
    ...coverageRows,
  ...
  const identityRow = identityEvidence(company);
  const sectorRow = sectorAssignmentEvidence({
    module,
    proposedModule,
    industryAssignment,
    assignmentApplied,
  ...
  const gapRows = gapEvidence(gaps);
  const evidenceItems = [
    identityRow,
    ...(sectorRow ? [sectorRow] : []),
    ...substantiveEvidenceItems,
    ...gapRows,
  ];
  ...
  const gate = baselineGate({
    ...
    evidenceCount: substantiveEvidenceItems.length,
  });
  ...
  const packWithoutHash = {
    ...
    sector_evidence: {
      ...
      assignment_source: assignmentApplied
        ? "research_factory_industry_assignments"
        : explicitReviewedModule
          ? "explicit_reviewed_input"
          : null,
      assignment_policy_version: industryAssignment?.policy_version ?? null,
      assignment_confidence: numberOrNull(industryAssignment?.confidence),
      assignment_decision_hash: industryAssignment?.decision_hash ?? null,
    ...
    universal_evidence: {
      required_metric_keys: universalRequiredKeys,
    ...
    provenance: {
      ...
      industry_assignment_id: industryAssignment?.id ?? null,
      ...
      evidence_gap_count: gapRows.length,
