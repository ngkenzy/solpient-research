...
function concentration(positions,totalValue){
  const weighted=positions.map(p=>({
    ...
    weight:totalValue?p.market_value/totalValue:0,
  })).sort((a,b)=>b.market_value-a.market_value||a.ticker.localeCompare(b.ticker));
  const hhi=weighted.reduce((s,p)=>s+p.weight*p.weight,0);
  return {
    largest_position_pct:round((weighted[0]?.weight??0)*100,2),
    top_3_pct:round(weighted.slice(0,3).reduce((s,p)=>s+p.weight,0)*100,2),
    hhi:round(hhi,4),
    effective_positions:hhi>0?round(1/hhi,2):0,
    positions:weighted.map(p=>({...p,weight_pct:round(p.weight*100,2)})),
  };
}
...
export function analyzePortfolio(input={},constraints={}){
  ...
  const portfolioBaseCagr=total&&investedBaseCagr!=null
  ...
  const base={
    ...
    positions,
    groups:groupExposure(positions,total),
    readiness:readinessExposure(positions,total),
