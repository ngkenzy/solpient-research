...
export const AUTONOMOUS_VALUATION_POLICY_VERSION="valuation-assumptions-v2.1";
...
function primarySourcePct(fundamentals=[]){
  ...
  const primary=fundamentals.filter(row=>
    row?.provider==="sec_companyfacts"||
    String(row?.source_url??"").includes("sec.gov")
  ).length;
...
export function buildAutonomousValuationPolicy({
  ...
  if(!industryModule){
    return{
      ...
      evidence:{},
      decision_hash:canonicalSha256({
        contract:"solpient-autonomous-valuation-policy-v2.1",
  ...
  const currentPrice=n(screenResult?.input_summary?.price);
  ...
  const valuationInput={
    ...
    evidence:{
      ...
      primarySourcePct:round(primarySourcePct(fundamentals),1),
    },
  };
  ...
  if(baseInitial==null)criticalIssues.push("insufficient_growth_evidence");
  if(!historical)criticalIssues.push("insufficient_historical_multiple_evidence");
  if(!peer)criticalIssues.push("insufficient_peer_multiple_evidence");
