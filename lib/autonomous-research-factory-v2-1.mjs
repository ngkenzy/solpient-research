...
export const AUTONOMOUS_SETTLED_QUEUE_VERSION="autonomous-settled-queue-v2.1.2";

const settlementMetricRows=(baselineDraft)=>(
  Array.isArray(baselineDraft?.draft_payload?.metric_observations)
    ?baselineDraft.draft_payload.metric_observations
    :[]
).map(row=>({
  metric_key:row?.metric_key??null,
  status:row?.status??null,
  value_numeric:n(row?.value_numeric),
  period_end:row?.period_end??null,
})).filter(row=>row.metric_key).sort((a,b)=>a.metric_key.localeCompare(b.metric_key));

const settlementPeerStructure=(contextPack)=>(
  Array.isArray(contextPack?.peer_comparison)?contextPack.peer_comparison:[]
).map(row=>({
  ticker:canonicalTicker(row?.ticker??row?.peer_ticker),
  data_status:row?.data_status??null,
  available_metrics:Object.entries(row?.metrics??{})
    .filter(([,value])=>n(value)!=null)
    .map(([key])=>key)
    .sort(),
})).sort((a,b)=>a.ticker.localeCompare(b.ticker));

export function buildAutonomousSettlementHash({
  item={},
  coverage=null,
  contextPack=null,
  consensus=null,
  ...
  return canonicalSha256({
    contract:"solpient-autonomous-settlement-v2.1.2",
    queue_version:AUTONOMOUS_SETTLED_QUEUE_VERSION,
    industry_policy_version:AUTONOMOUS_INDUSTRY_POLICY_VERSION,
    valuation_policy_version:AUTONOMOUS_VALUATION_POLICY_VERSION,
    ticker:canonicalTicker(item?.ticker),
    source_screen_result_id:item?.source_screen_result_id??null,
    screen_result_hash:item?.state_snapshot?.screen_result_hash??null,
    factory_state_hash:item?.state_hash??null,
    coverage:coverage?{
      status:coverage.status??null,
      fundamentals_pct:n(coverage.fundamentals_pct),
      history_pct:n(coverage.history_pct),
      market_history_pct:n(coverage.market_history_pct),
      valuation_history_pct:n(coverage.valuation_history_pct),
      capital_allocation_pct:n(coverage.capital_allocation_pct),
      consensus_pct:n(coverage.consensus_pct),
      industry_pct:n(coverage.industry_pct),
      peer_pct:n(coverage.peer_pct),
      overall_pct:n(coverage.overall_pct),
      decision_readiness_pct:n(coverage.decision_readiness_pct),
      normalized_quarters:n(coverage.normalized_quarters),
      complete_fiscal_years:n(coverage.complete_fiscal_years),
      market_days:n(coverage.market_days),
      primary_source_quarters:n(coverage.primary_source_quarters),
      missing_fields:coverage.missing_fields??[],
      provider_summary:coverage.provider_summary??{},
    }:null,
    context:contextPack?{
      industry_module:contextPack.industry_module??null,
      history_coverage:contextPack.history_coverage??{},
      peers:settlementPeerStructure(contextPack),
      summary:contextPack.summary?{
        historical_metric_rows:n(contextPack.summary.historical_metric_rows),
        valuation_history_rows:n(contextPack.summary.valuation_history_rows),
        capital_allocation_rows:n(contextPack.summary.capital_allocation_rows),
        configured_peers:n(contextPack.summary.configured_peers),
        peers_with_local_data:n(contextPack.summary.peers_with_local_data),
        full_fiscal_years:n(contextPack.summary.full_fiscal_years),
      }:null,
      limitations:contextPack.limitations??[],
    }:null,
    consensus:consensus?{
      analyst_count:n(consensus.analyst_count),
      revenue_next_fy:n(consensus.revenue_next_fy),
      eps_next_fy:n(consensus.eps_next_fy),
      revenue_growth_next_fy:n(consensus.revenue_growth_next_fy),
      eps_growth_next_fy:n(consensus.eps_growth_next_fy),
    }:null,
    baseline:baselineDraft?{
      industry_module:baselineDraft.industry_module??null,
      evidence_completeness_pct:n(baselineDraft.evidence_completeness_pct),
      standard_status:baselineDraft.standard_status??null,
      metrics:settlementMetricRows(baselineDraft),
    }:null,
    industry_assignment:industryAssignment?{
      policy_version:industryAssignment.policy_version??null,
      status:industryAssignment.status??null,
      module:industryAssignment.module??null,
      proposed_module:industryAssignment.proposed_module??null,
      decision_hash:industryAssignment.decision_hash??null,
    }:null,
  });
}

export function selectAutonomousResearchFactoryCandidates({
  items=[],
  decisions=[],
  currentSettlementHashes=new Map(),
  priorSettlementHashes=new Map(),
  ticker=null,
  maxItems=10,
}={}){
  const requested=ticker?canonicalTicker(ticker):null;
  const latestCurrentValuationDecision=new Set(
    (decisions??[])
      .filter(row=>
        row?.decision_type==="valuation_assumptions"&&
        row?.policy_version===AUTONOMOUS_VALUATION_POLICY_VERSION
      )
      .map(row=>row.research_factory_item_id)
  );

  const candidates=[];
  for(const item of items??[]){
    const symbol=canonicalTicker(item?.ticker);
    if(requested&&symbol!==requested)continue;
    if(
      item?.status==="complete"||
      item?.stage==="complete"||
      item?.stage==="pipeline_refresh"||
      item?.stage==="research_review"
    )continue;
    if(item?.status==="running")continue;

    if(requested){
      candidates.push({...item,selection_reason:"manual_ticker_override"});
      continue;
    }
    ...
    if(item?.status==="quarantined"){
      const current=currentSettlementHashes.get(item.id)??null;
      const prior=priorSettlementHashes.get(item.id)??null;
      if(!current)continue;
      if(prior&&prior===current)continue;
      candidates.push({
        ...item,
        selection_reason:prior?"quarantine_evidence_changed":"quarantine_recheck_under_settled_queue",
      });
      continue;
    }

    if(item?.status==="needs_review"){
      if(latestCurrentValuationDecision.has(item.id))continue;
      candidates.push({...item,selection_reason:"unattempted_review_gate"});
      continue;
    }

    if(item?.status==="queued"){
      candidates.push({...item,selection_reason:"queued_unattempted"});
    }
  }

  const priority={
    unattempted_review_gate:0,
    queued_unattempted:1,
    quarantine_evidence_changed:2,
    quarantine_recheck_under_settled_queue:3,
    manual_ticker_override:0,
  };
  return candidates
    .sort((a,b)=>
      (priority[a.selection_reason]??9)-(priority[b.selection_reason]??9)||
...
function moduleFromScreen({ticker,screenProfile,sector,industry}={}){
  ...
  if(sectorText.includes("energy")||industryText.includes("oil")||industryText.includes("gas")){
    return{module:"energy_integrated",rule:"sector:energy",ruleConfidence:.92};
  }
  if(sectorText.includes("real estate")||industryText.includes("reit")){
    return{module:"real_estate_reit",rule:"sector:real_estate",ruleConfidence:.90};
  }
  if(sectorText.includes("financial")){
    if(
      industryText.includes("bank")||
      industryText.includes("credit")||
      industryText.includes("thrift")
    ){
      return{module:"financial_bank",rule:"financial:bank",ruleConfidence:.94};
    }
    if(industryText.includes("insurance")){
      return{module:"financial_insurance",rule:"financial:insurance",ruleConfidence:.94};
    }
    return{module:"financial_services",rule:"sector:financial_services",ruleConfidence:.84};
  }
...
function moduleRiskRate(module){
  const map={
    ...
    financial_insurance:9.25,
    financial_services:9.5,
    energy_integrated:10.5,
    real_estate_reit:9.75,
...
function multipleSeries({industryModule,valuationHistory=[]}={}){
  const useForwardPe=["biopharma","financial_bank","financial_insurance","financial_services"].includes(industryModule);
