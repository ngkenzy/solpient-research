...
export async function buildReferencePeerContext({ticker,asOfDate=new Date().toISOString().slice(0,10),userAgent=process.env.YAHOO_DATA_USER_AGENT??"SOLPIENT Research/1.0"}){
    ...
    try{
      ...
      for(const [metric_key,value] of Object.entries(metrics)){
        ...
        snapshotRows.push({
          ...
          provider:"yahoo-reference-peer-v1",source_url:market.source_url,observed_at:new Date().toISOString()
        });
      }
