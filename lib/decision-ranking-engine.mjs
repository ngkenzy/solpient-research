...
export function currentPriceExpectedCagr(price,scenario={}){
  const p=n(price);
  const horizon=n(scenario?.horizon_years);
  const terminal=n(
    scenario?.estimated_terminal_value_per_share ??
    scenario?.terminal_value
  );
  const dividends=n(
    scenario?.assumptions?.dividends_assumed ??
    scenario?.return_decomposition?.dividends_assumed
  ) ?? 0;
  if(p!=null&&p>0&&horizon!=null&&horizon>0&&terminal!=null&&terminal+dividends>0){
    return round1(
      (Math.pow((terminal+dividends)/p,1/horizon)-1)*100
    );
  }
  return n(scenario?.expected_cagr);
}
...
export function readinessState({
  ...
  const researchReady=
    blockers.length===0 &&
    atLeast(evidence?.score,60) &&
    atLeast(coverage.research_structure_pct,80) &&
    atLeast(coverage.fundamentals_pct,70) &&
    atLeast(coverage.history_pct,60);

  if(!researchReady){
    if(evidence?.score!=null&&evidence.score<60)warnings.push("Evidence Confidence is below the 60% Research Ready threshold.");
    return {
