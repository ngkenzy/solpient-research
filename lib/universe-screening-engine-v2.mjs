export const UNIVERSE_SCREENING_VERSION="solpient-universe-screen-v2";
export const UNIVERSE_SELECTION_VERSION="solpient-100-selection-v2";
...
export function screenCompany(row={}){
  ...
  const result={
    ...
    sectorTaxonomyVersion:row.sector_taxonomy_version??null,
    ticker:String(row.ticker??"").toUpperCase(),
    companyName:row.company_name??row.companyName??null,
    sector:row.sector??null,industry:row.industry??null,profile:profileName,
    screenScore:score,qualityCoreScore:qualityScore,evidenceCoveragePct:coverage,state,gates,
    dimensions:{
