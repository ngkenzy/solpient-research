...
} from "./universe-screening-engine-v2-2.mjs";
import {
...
export const RESEARCH_CANDIDATE_PIPELINE_VERSION="research-candidate-pipeline-v2.2";
...
function nextActions({eligible,companyExists,preflight,valuation,decision,sectorEvidence}){
  const actions=[];
  if(!eligible){
    return [{
      priority:100,
  ...
  if(sectorEvidence?.candidatePromotionBlocked&&sectorEvidence?.blocker){
    actions.push({
      priority:99,
      type:"classification",
...
export function buildResearchCandidatePipeline({
  ...
  const actions=nextActions({
    eligible,companyExists,preflight,valuation:valuation??{},decision,sectorEvidence,
  });
