import { loadCompanyIntelligenceData } from "@/lib/repositories/company-intelligence";
...
export async function CompanyIntelligence({ ticker, asOf = null }: { ticker: string; asOf?: string | null }) {
  const loaded = await loadCompanyIntelligenceData(ticker, asOf);
  if (!loaded) return null;

  const { company, rows, coverageChecks, providerHealth } = loaded;
